...
**Groq API** と GAS で作るとする。

**Groq API**: GAS から `UrlFetchApp` で Groq の **OpenAI 互換エンドポイント** を呼ぶ。モデルは **Llama 4 Scout**（`meta-llama/llama-4-scout-17b-16e-instruct`）など、ビジョン対応モデルを使用する。1枚500KB程度の画像・**1リクエスト最大5枚**・JSON mode 対応済みなので、本仕様のプロンプトで処理できる。思考モードは使えないが、説明文生成（画像→JSON）も、表題・目次・ワークフロー・FAQ 付与（JSON→JSON）も Groq API だけで実装可能。

**Groq で画像→説明を並列実行する場合**: Groq の公式 Rate Limits（[GroqDocs](https://console.groq.com/docs/rate-limits)）では、**同時接続数（concurrency）の上限は明示されていない**。制限は **RPM（Requests Per Minute）** と **TPM（Tokens Per Minute）** で、Llama 4 Scout の Developer プランでは **RPM=30**、RPD=1,000、TPM=30,000、TPD=500,000。つまり「1分あたり最大30リクエスト」まで。同じ1分以内に送れるリクエストは最大30本なので、**画像を読んで説明を返す API 呼び出しは、最大で 30 並列まで**（30 バッチを同時に投げるとその分で RPM を消費）。本仕様の「5枚で1回」なら、20枚＝4リクエストなので 4 並列で余裕。画像が多くても、バッチ数が 30 以下ならその数だけ並列可能。Enterprise や Flex ではより高い制限が用意されている場合あり。
...
**API キーをユーザーが自分で取得して Web に入れる場合**: API キーをユーザー各自が取得し、画面で入力してもらう運用にすると、利用はそのユーザーの Groq アカウントに紐づく。レート制限はキー（アカウント）ごとなので、他人の利用で自分の枠が減ることはなく、1ユーザーあたり「20枚＝4リクエスト」程度なら無料枠でも余裕で収まる。サーバー側で1本のキーを共有するより、ユーザー持ちキーにした方が制限に余裕ができて安定して動く。
...
## マニュアル作成（画像→文章）の無料枠: Groq を前提とした目安

Groq 無料で使える範囲での目安。公式の最新数値は [Groq Limits](https://console.groq.com/settings/limits) で要確認。

| 項目 | Groq（無料枠・Llama 4 Scout 等） |
|------|-----------------------------------|
| **主なモデル** | Llama 4 Scout 17B（ビジョン対応） |
| **画像入力** | 対応。1リクエスト最大 **5枚**。1枚あたり 20MB(URL) または リクエスト合計 4MB(base64)。 |
| **RPM（1分あたりリクエスト）** | **30** 前後（Llama 4 Scout。アカウントにより異なる） |
| **RPD（1日あたりリクエスト）** | **1,000** 前後（モデル・アカウントで要確認） |
| **TPM（1分あたりトークン）** | 6,000〜30,000 程度（Free は低めの可能性） |
| **思考モード** | なし |
| **JSON / 構造化出力** | 対応（JSON mode、Structured Outputs） |
| **日本語** | 利用可能 |
| **制限の単位** | アカウント（APIキー）単位 |
| **本仕様の目安（20枚→4リクエスト）** | 4 リクエストは 30 RPM 内で余裕。RPD 1K なら 1 日 250 回程度まで同規模が目安。 |

**オプション（品質を上げたい場合）**: 最終文章の校整だけ **Gemini** を 1 回使うハイブリッド案も選択肢として持てる。回数が多い画像→文章は Groq、最後に JSON を 1 回だけ Gemini に送って「日本語を自然に校整する」と依頼する構成。その場合は Groq 用と Gemini 用の両方の API キー入力（または片方だけ使うモード）を考慮する。
...
- 例: デフォルトは「5枚で1回」（画像20枚ならAPI 4回）。「1枚で1回」などに変更可能。APIの負荷や確実さのトレードオフで選べるようにする。Groq は 1 リクエスト最大 5 枚なので、選択肢は 1〜5 の範囲とする。

1. 人間に画像をアップロードさせる画面からアップロードする　「説明文生成」ボタンをクリックして実行開始
2. 以下のプロンプトで、**Groq API**（OpenAI 互換エンドポイント）を呼び、説明文のJSONを作成する
...
この時も **Groq API** を呼び、以下のプロンプトで表題・目次・ワークフロー・FAQ を付与する。
 ...
 *   2. 中身をすべて削除し、Groq などで作ったJSONを貼り付ける（これだけ）。
...
Groq API と GAS の組み合わせで、仕様に書かれている流れは実現できます。
...
| 項目 | 評価 |
|------|------|
| 画像アップロード＋「説明文生成」 | 可能。GAS の HtmlService や Web アプリでアップロードを受け取り、画像を Drive 等に保存してから Groq（OpenAI 互換エンドポイント）に渡せばよい。 |
| 画像→説明文 JSON（1〜2のプロンプト） | 可能。Groq の画像入力＋JSON mode で、仕様の JSON 形式を返させることは現実的。1リクエスト最大5枚なので、5枚ずつバッチで呼ぶ。 |
| 画像と説明を左右に並べて表示・編集（3〜4） | 可能。HTML で 1 枚ごとに「左：画像」「右：説明」を出し、テキストを編集可能にすればよい。 |
| 表題・目次・ワークフロー・FAQ の付与（5〜7のプロンプト） | 可能。既存の slides JSON を入力に、cover / toc / workflow / faq を追加する Groq の LLM 呼び出しで対応できる。 |
...
2. **画像枚数と Groq の制限**  
   - 1リクエスト最大 **5枚** なので、デフォルトは「5枚で1回」の API 呼び出しとする。  
   - **オプションで変更可能にする**: 1枚で1API、3枚で1API、5枚で1API などをユーザーが選べるようにする（仕様の「オプション」に記載済み）。RPM 以下に並列度を抑える。

3. **Groq の返答が JSON 以外になる場合**  
...
- フロー全体（アップロード → 説明生成（Groq）→ 確認・編集 → 表題・目次・ワークフロー・FAQ 付与（Groq）→ 新規スライド作成 → 表示）は**実装可能**です。  
...
| **画像の最大枚数** | プロンプトで「1.png〜20.png」とあるのみ | 上限は 20 枚固定か、可変か。可変なら上限値（例: 50 枚）を決めるか。Groq は 1 リクエスト5枚なので、バッチ数が RPM を超えないようにする。 |
| **バッチオプションの UI** | 「5枚で1回」「1枚で1回」と例のみ | 選択肢は 1 / 3 / 5（Groq は最大5枚）を用意するか。設定は画面のドロップダウンか、設定ファイルか。 |
| **Groq API キー** | 未記載 | どこに保持するか（GAS の Script Properties、環境変数、ユーザー入力のみで保存しない など）。 |
...
| **エラー時** | 未記載 | 説明文生成失敗時・429（RPM/TPM 制限）時の表示やリトライ方針。`retry-after` の扱い。 |
